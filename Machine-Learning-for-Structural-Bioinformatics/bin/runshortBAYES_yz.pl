...
#    $recorddir/SVMcon$line".".map
#    $recorddir/svmseq.dat
#    $recorddir/SPcon$line.map
#    $recorddir/psicov$line.map
#    $recorddir/CCMpred$line.map
#    $recorddir/freecontact$line.map
#    $recorddir/meta-psicovstage2$line.map
#
# output
#    $recorddir/shortbayes$s
#############################
...
for($uiopp=0;$uiopp<1;$uiopp++){
    ...
    print "psicov= $rcddir/$pdbname\n";
    if(!-e "$rcddir/$pdbname"){
	print "$line	WARNING!!!!!!!!!!!!!!!!!!!!!!!!PSICOV \n";
    }
    open(GGG,"$rcddir/$pdbname");
    $numbpsicovini=0;
    while($para1=<GGG>){
    ...
    if(! -e "$rcddir/$pdbname"){
	print "$line	WARNING!!!!!!!!!!!!!!!!!!!!!!!! freecontact\n";
    }
    open(GGG,"$rcddir/$pdbname");
    $numbpsicovini=0;
    while($para1=<GGG>){
    ...
    if(!-e "$rcddir/$pdbname"){
	print "$line  WARNING!!!!!!!!!!!!!!!!!!!!!!!! meta-psicovstage2\n";
    }
    open(GGG,"$rcddir/$pdbname");
    $numbpsicovini=0;
    while($para1=<GGG>){
	...
	for($j=$i+1;$j<=$note;$j++){
		...
		$temp=0.099441346*betacon($pred[0][$i][$j])*svmcon($pred[1][$i][$j])*svmseq($pred[2][$i][$j])*spcon($pred[3][$i][$j])*psicov($pred[4][$i][$j])*ccmpred($pred[5][$i][$j])*evfold($pred[6][$i][$j])*meta($pred[7][$i][$j])/0.900558654;
		#print "aaa $temp\n";
		$temp=1-1/($temp+1);
		#print "bbaaa $temp\n";
		if($temp>1){
