...
$svmscaledir=$ARGV[3]; ## Added
$summarydir=$ARGV[4]; ## Added
...
#################################################
    ...
    for($j=0;$j<$len_protein;$j++)
    {
	  ...
	  if($kkkk % $cut_kkkk == ($cut_kkkk-1)){	      
	      close(fl3);
	      close(fl4);
	      `$svmscaledir/svm-scale -l -1 -u 1 -d $norm -r $summarydir/par$cut inputCA$cut.dat$mykkkk > scaled_inputCA$cut.dat$mykkkk `;
