...
# usage: preNN_yz.pl $s $rcddir

# this program is to generate feature files: $range-$s
...
foreach $line (@lists){
    ...
    ############################ read BAYES probability as trainign feature ########################
    ...
    ########## read solvant accessibility as feature ###############################
    @solve=();
	...
	open(WWW,">$rcddir/$range-$line");
	open(GGG,">$rcddir/label$line-$range");
	for($k=1;$k<$Lch-1;$k++){
	    for($j=$k+1;$j<=$Lch;$j++){
