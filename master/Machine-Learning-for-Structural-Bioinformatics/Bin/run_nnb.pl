...
################ 
$wekadir=$ARGV[0]; ## Changed "/nfs/amino-library/contact/NEW/weka-3-7-12"
$bindir=$ARGV[1]; ## Changed from "/nfs/amino-home/zhng/pdbinput/contact/bin"

#############################
#run BAYES >shortbayes$s "$i $j $p($i,$j)", basically convert original probability to unified probability
print "generate short bayes ......\n";
system("perl $bindir/runshortBAYES.pl $s $rcddir"); # > shortbayes$s
...
system("perl $bindir/runlongBAYES.pl $s $rcddir");
...
#################################################################################
###############################################
system("$bindir/preNN_yz.pl $s $rcddir"); #create feature files
...
`sed -i "s/*//g" $rcddir/$s-short-NN-BAYES`; #short-range NN-BAYES contact prediction

print "1111111aaa\n";
############ NN for medium-range CA-contact #####################################################
...
############ NN for medium-range CB-contact #####################################################
...
#########################################BETASTRANDS############################
`perl $bindir/BETASTRAND.pl $s $rcddir`; ## Changed

#^^^^^^^^^^ NN-BAYES complete ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

########### output NN-BAYES results -------------------->
################## sort contacts ####################
